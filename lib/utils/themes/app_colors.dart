import 'package:flutter/material.dart';

class AppColors {
  AppColors._();


  ///Dark Theme
  static const greyBackground = Color(0xFF373A40);


  ///Light Theme
  static const whiteBackground = Color(0xffF7F8FB);
  static const buttonWhite = Color(0xFFF6F7F8);
  static const buttonBlue = Color(0xFF458DBE);
  static const buttonTagColor = Color(0xFFEEEEEE);

  /// TextColors
  static const darkBlueTextColor = Color(0xFF002561);
  static const alertRedTextColor = Color(0xFFCD716F);

  /// Accent colors for light theme
  static const blackBlue = Color(0xFF1A2130);
  static const shadowColor2 = Color(0xFF323B62);
  static const primaryBlue = Color(0xFF1463F5);
  static const primaryBlueLight = Color(0xFF7DA8FA);
  static const secondaryBlue = Color(0xFFE5EDFF);
  static const pureWhite = Color(0xFFFFFFFF);
  static const orange = Color(0xFFff9248);
  static const lightOrange2 = Color(0xFFFDBF60);
  static const lightOrange = Color(0xFFffd7b5);


  /// Alert Colors
  static const alertRed = Color(0xFFec2d42);
  static const alertRed2 = Color(0xFFfa152e);
  static const alertRed3 = Color(0xFFF1D7D7);


}