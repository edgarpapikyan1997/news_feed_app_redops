...
class PostBloc extends Bloc<PostEvent, PostState> {
  ...
  PostBloc(this._postRepository) : super(const PostLoadingState()) {
    on<LoadPostEvent>((event, emit) async {
      if (state is PostLoadedState && (state as PostLoadedState).hasReachedMax) {
        return;
      }

      try {
        if (state is PostLoadingState) {
          final posts = await _postRepository.getAllPosts(count: event.count, startIndex: event.startIndex);
          emit(PostLoadedState(posts: posts, hasReachedMax: posts.length < count));
